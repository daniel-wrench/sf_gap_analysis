...
# plt.rcParams["mathtext.default"] = "regular"  # ✅ Fixes MathText font override
...
fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(8, 7), sharey=True)
ax1, ax2 = axes
...
ax1.annotate(
    "$\\textbf{HELIOSHEATH}$",
    xy=(v1_hp_date - pd.DateOffset(months=30), 0.02),
...
ax1.annotate(
    "$\\textbf{VERY\ LOCAL\ INTERSTELLAR\ MEDIUM}$",
    xy=(v1_hp_date + pd.DateOffset(days=120), 0.02),
    xycoords=("data", "axes fraction"),
    fontsize=10,
    ha="left",
    va="bottom",
...
# ax2.annotate(
#     "$\\textbf{HELIOSHEATH}$",
#     xy=(v2_hp_date - pd.DateOffset(months=25), 0.02),
#     xycoords=("data", "axes fraction"),
#     fontsize=12,
#     fontweight="bold",
#     ha="left",
...
ax1.set_xlabel("Date")
ax = plt.gca()
# ax1.ticklabel_format(useOffset=False, style="plain")  # good for numeric formatting


handles, labels = ax1.get_legend_handles_labels()
# handles = [handles[0], handles[2], handles[1]]
labels = [r"$|\textbf{B}|$", r"$B_R$", r"$B_T$", r"$B_N$"]

ax1.legend(handles, labels, loc="upper left", fontsize=10)

# Set the y-axis upper limit to make room for the annotation
# ax2.set_ylim(top=ax2.get_ylim()[1] * 1.4)
ax1.set_ylim(-0.8, 0.9)

# Align the x-axis for both plots
ax1.set_xlim([v1_hp_date + pd.DateOffset(months=-33), v1_hp_date + v1_time_diff])
...
ylim = ax1.get_ylim()
ax1.text(
    v1_hp_date - pd.DateOffset(days=120),
    ylim[1] * 0.85,
    "$\\textbf{hp}$",
    rotation=90,
    verticalalignment="top",
...
plt.tight_layout()
plt.savefig("voyager_lism_data_OLD_WAY.pdf")
print("Done")
