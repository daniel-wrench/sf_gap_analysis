...
v2_hr = v2_raw.resample("1h").mean()

v2_hr_sub = v2_hr.loc["2021-01-01":"2021-06-20"]
# Standardize the data
# v2_raw_sub_hr = (v2_raw_sub_hr - v2_raw_sub_hr.mean()) / v2_raw_sub_hr.std()

v2_hr_sub["Radius"].interpolate(method="linear", inplace=True)
v2_hr_sub.set_index("Radius", inplace=True)

v2_hr_sub[["BR", "BT", "BN"]].plot(figsize=(8, 3))
plt.legend(
    loc="center right",
    fontsize=12,
)

v2_raw["Radius"].interpolate(method="linear", inplace=True)
v2_raw.set_index("Radius", inplace=True)
v2_raw_sub = v2_raw.loc[126.4:126.5]
