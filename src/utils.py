...
def plot_results(
    predictions_arr, observed_arr, no_samples, spacecraft, model, log=False
):
    ...
    if log == True:
        for i in np.arange(0, no_samples):
            r, c = (
                ...
                np.mod(np.arange(0, no_samples)[i], nplotx),
            )
...
def plot_validation_error(path):
    ...
    plt.title(
        "Min validation loss of "
        + str(round(min_loss.iloc[0, 0], 4))
        + " at epoch "
        + str(min_loss.index[0]),
        fontsize=10,
    )
